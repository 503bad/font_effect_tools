...
│   │                    glow, slice, slime, wave, storyteller,
│   │                    cartoon の全 23 ファイル）
...
- **エフェクト選択ドロップダウン**で表示効果を切り替えます。現在は次の 34 種類:
  ...
  - **フリップ (Flip)** — 各文字が**その場で疑似的に立体的な横回転**（Y軸スピン）
    をします。普段は静止して読める状態のまま、**回転頻度**で指定した間隔ごとに
    360° 回転の波が**左から順・右から順・ランダム**のいずれかの順序で走ります。
    回転の後半（裏面）は**左右反転した文字をやや暗く**表示して立体感を出します。
    文字色・輪郭線・グロー・ドロップシャドウを設定できます。
  - **ストーリーテラー (Storyteller)** — スター・ウォーズ冒頭のような**パース付きの
    クロール**。テキストが手前（下）から消失点（画面上部の地平線）へ向かって
    奥へ流れていきます。遠ざかるほど横幅が狭まり暗くなります。流れる速さ・
    フェードイン/アウトのオンオフ・ループ/ワンショットを設定できます。
  - **フォント切り替え (Font switch)** — 同じテキストを**指定した複数のフォント**で
    ラスタライズし、時間経過で切り替えます。切り替えは**順番**または**ランダム**、
    切り替わるまでの**間隔（秒）**を指定できます。フォントは一覧（改行／カンマ
    区切り）で複数指定し、各フォントのテキスト帯の中心が揃うように配置されます。
  - **ころころ (Korokoro)** — 各文字が 1 文字ずつ**転がりながらバウンドして
    フェードイン**してきます。速さ・バウンドの大きさ・転がる方向（左から／
    右から）・ループ/ワンショット・**転がって出ていくか**・フェードイン／
    フェードアウトを設定できます。
  - **アウトライナー (Outliner)** — 発光するペンが**フォントの輪郭をなぞって**いき、
    緩やかに消えていく**尾**を引きます（ラインのみ表示・塗りなし）。FreeType の
    グリフ輪郭を抽出して 1 本のパスにつなぎ、ペンを一定速度で走らせます。回る向き
    （右回り／左回り）・速さ・ラインの太さ・尾の長さ／色／寿命・ブルーム・グローを
    設定できます。
  - **パズル (Puzzle)** — テキストを**縦横の格子に分割したパーツ**が四方から
    スライドインして組み上がります。パーツの細かさ・速さ・スライドインの順番
    （左から／右から／全体）・ブルーム・グローを設定できます。
  - **カートゥーン (Cartoon)** — レトロなカートゥーンのように**低いフレームレートで
    カクカク歪む「ラインボイル」**。1 フレームごとにスナップする空間ノイズで輪郭が
    手描き風に揺れ、エッジに**にじみ（インクのにじみ）**が乗ります。かくかく歪む
    周期・歪む強度・にじみを設定できます。
...
| `src/effect-flip.c/.h` | フリップ（文字単位の疑似3D横回転・順序選択） |
| `src/effect-storyteller.c/.h` | ストーリーテラー（地平面へ向かうパース付きクロール） |
| `src/effect-fontswitch.c/.h` | フォント切り替え（複数フォントを自前でラスタライズして時間切替） |
| `src/effect-roll.c/.h` | ころころ（文字単位の転がりバウンド＋ロール回転） |
| `src/effect-outliner.c/.h` | アウトライナー（FreeType 輪郭抽出＋発光する尾のなぞり） |
| `src/effect-puzzle.c/.h` | パズル（テキストを格子分割して四方からスライドイン） |
| `src/effect-cartoon.c/.h` | カートゥーン（低フレームレートのラインボイル＋にじみ） |
...
| `data/effects/storyteller.effect` | テキスト帯を地平面へ投影するパースクロールのシェーダー |
| `data/effects/cartoon.effect` | フレーム量子化の空間ノイズ歪み＋エッジ膨張／にじみのシェーダー |
| `data/effects/puzzle.effect` | パーツごとに頂点カラーを持つマスク塗りのシェーダー（一括描画） |
...
| プロパティ | 範囲 | 既定値 |
| --- | --- | --- |
| エフェクト | 全 34 種類（上記「できること」参照・パネル先頭に配置） | 炎 |
| テキスト | — | `Font Effect Tools` |
| フォント（システムピッカー） | — | Impact |
| 行間 | オート / ピクセル指定（1 – 2000 px・ベースライン間隔） | オート |
| 文字間 | オート / ピクセル指定（-500 – 1000 px・送り幅に加算） | オート |
| 書字方向 | 横書き / 縦書き（ラジオボタン） | 横書き |
| 揃え位置 | 左揃え / 中央揃え / 右揃え（縦書き時は 上揃え / 中央揃え / 下揃え） | 中央揃え |

> - **行間**のピクセル指定はベースライン間隔の絶対値、**文字間**のピクセル指定は
>   フォント本来の送り幅に加算する値です（負の値で詰められます）。
> - **揃え位置**は複数行テキストの各行をブロック内で左／中央／右に揃えます。
> - **書字方向**を**縦書き**にすると、文字は各列を上から下へ進み、改行で新しい列が
>   でき、列は**右から左**へ並びます（縦組み）。字送りは 1 文字 = 1 em ＋ 文字間、
>   **行間**の指定は**列の間隔**として働きます。揃え位置は各列を上／中央／下に
>   揃える意味になり、ラベルも自動で切り替わります。約物（「ー」や括弧など）の
>   回転・縦書き用グリフへの置換は行いません。
...
> **フェードイン / フェードアウト**は、文字が跳ねながら入ってくるとき／出ていく
> ときに透明度をなめらかに変化させます。オフにすると従来どおり不透明のまま
> 出入りします。
...
| --- | --- | --- |
| フォントの色 | カラーピッカー | 白 `#FFFFFF` |
| 回転の順序 | 左から順 / 右から順 / ランダム | 左から順 |
| 回転頻度 (回/秒) | 0.05 – 3.0 | 0.5 |
| グロー | 0.0 – 3.0 | 0.0 |
| 輪郭線を表示 | オン / オフ | オフ |
| 輪郭線の太さ (px) | 1.0 – 20.0 | 4.0 |
| 輪郭線の色 | カラーピッカー | 黒 `#000000` |
...
> 各文字は普段静止しており、**回転頻度**ごとに 360° の横回転（Y軸スピン）の波が
> **回転の順序**で指定した順に走ります。回転の裏面（90°〜270°）は左右反転した
> 文字をやや暗く表示します。輪郭線・グロー・シャドウは回転に追従します。

### ストーリーテラー (Storyteller)

| プロパティ | 範囲 | 既定値 |
| --- | --- | --- |
| 文字の色 | カラーピッカー | 淡い金 `#FFD86B` |
| 流れる速さ | 0.05 – 1.5 | 0.30 |
| フェードイン / アウト | オン / オフ | オン |
| ループ | オン / オフ | オン |

> テキストは画面下（手前）から上部の地平線（消失点）へ向かって奥へ流れます。遠ざかる
> ほど横幅が狭まり暗くなります。**フェードイン / アウト**をオンにすると、手前で現れる
> ときと遠くへ消えるときに透明度がなめらかに変化します。**ループ**オフでは一度流れ
> きると終了します。

### フォント切り替え (Font switch)

| プロパティ | 範囲 | 既定値 |
| --- | --- | --- |
| フォントの色 | カラーピッカー | 白 `#FFFFFF` |
| 切り替えフォント 1 – 8 | フォントピッカー | 1=Impact / 2=Arial / 3=Georgia / 4–8=未設定 |
| 切り替え順 | 順番 / ランダム | 順番 |
| 切り替え間隔 (秒) | 0.1 – 10.0 | 1.0 |

> 切り替えたいフォントを**フォントピッカー（スロット 1〜8）**で選びます。設定された
> スロットだけが使われ、空のスロットは無視されます。共通の「テキスト」「フォント
> サイズ」「行間／文字間／揃え位置／書字方向」を使って各フォントを自前でラスタライズし、
> 各フォントのテキスト帯の中心が揃うように重ねて表示します（各スロットの太字／斜体は
> 反映されます）。解決できなかったフォントはスキップされ、有効なフォントが 1 つも
> ないときは共通フォントのテキストをそのまま表示します。
...
> 各文字が指定方向から 1 文字ずつ転がり込み、車輪のように移動距離に応じて回転しながら
> 減衰バウンドしてフェードインします。**転がって出ていく**オンで、待機後に同じ方向へ
> 転がって退場します（オフなら待機後その場でフェードアウト）。**ループ**オフかつ
> 退場オフでは、入ってきて整列したまま静止します。

### アウトライナー (Outliner)

| プロパティ | 範囲 | 既定値 |
| --- | --- | --- |
| ラインの色 | カラーピッカー | 白 `#FFFFFF` |
| 回る向き | 右回り / 左回り | 右回り |
| 速さ | 0.1 – 3.0 | 1.0 |
| ラインの太さ (px) | 1.0 – 30.0 | 6.0 |
| 尾の長さ (輪郭の割合 %) | 1 – 100 | 25 |
| 尾の寿命 (秒) | 0.2 – 10.0 | 2.0 |
| グロー | 0.0 – 3.0 | 1.0 |
| ブルーム | 0.0 – 3.0 | 1.0 |

> FreeType でグリフの輪郭を抽出して 1 本のパスにつなぎ、発光するペンを一定速度で
> 走らせます。塗りは描かず、**ラインのみ**を表示します。ペンの後ろには加算合成の
> やわらかい**尾**が伸び、緩やかに薄れて消えます。**尾の長さ**は輪郭パス全体に対する
> 割合（%）で指定し、**尾の寿命**でも長さが制限されます（実際の尾＝両者の短い方。
> 長い尾にするには割合と寿命の両方を上げてください）。**グロー**は芯の明るさ、
> **ブルーム**は外周のにじみです。
...
| パーツの細かさ | 2 – 20 | 6 |
| 速さ | 0.2 – 3.0 | 1.0 |
| スライドインの順番 | 左から / 右から / 全体 | 左から |
| グロー | 0.0 – 3.0 | 0.0 |
| ブルーム | 0.0 – 3.0 | 0.0 |
| ループ | オン / オフ | オン |
| ループ待機時間 (秒) | 0.0 – 10.0 | 1.5 |

> テキスト帯を**パーツの細かさ**（列数）に応じた格子に分割し、各パーツが四方
> （左右上下）からスライドインしてフェードしながら定位置に組み上がります。
> **スライドインの順番**は、左から順・右から順・全体（各パーツがランダムな遅延で
> いっせいに）から選べます。**ループ**オンでは「組み上がり → 待機 → 分解（逆再生）」
> を繰り返し、オフでは一度組み上がったまま静止します。
...
| --- | --- | --- |
| フォントの色 | カラーピッカー | 白 `#FFFFFF` |
| かくかく周期 (秒) | 0.03 – 0.5 | 0.12 |
| 歪む強度 | 0.0 – 1.0 | 0.35 |
| にじみ | 0.0 – 1.0 | 0.3 |

> 時間を**かくかく周期**で量子化した空間ノイズで輪郭を歪ませ、1 フレームごとに
> 形がスナップする手描きアニメ風の「ラインボイル」になります。**かくかく周期**を
> 小さくすると速く・大きくするとゆっくりカクつきます。**歪む強度**は揺れの量、
> **にじみ**はエッジの膨張とインクのにじみの強さです。歪み／にじみが切れないよう
> 余白を全方向に自動確保します。
...
への整形配置 → ローカル OBS への配置**までを一括で行います。

```
build.bat            ビルド（差分）して package\ を最新化し、OBS へ配置
...
> 入力してください。
>
> **OBS への配置について:** 仕上げに `package\` の内容を OBS のインストール先
> （既定 `C:\Program Files\obs-studio`）へコピーします。
> `package\font-effect-tools\` → `…\data\obs-plugins\font-effect-tools\`、
> `package\bin\` の DLL/PDB → `…\obs-plugins\64bit\` です。`Program Files` への
> 書き込みには**管理者権限**が必要なので、**「管理者として実行」したコマンド
> プロンプトから、OBS を閉じた状態で** `build.bat` を実行してください（権限が
> ない場合はビルド／package 化までは成功し、配置のみスキップ／失敗します）。
> OBS の場所が異なるときは `build.bat` 冒頭の `OBS_DIR` を編集してください。
