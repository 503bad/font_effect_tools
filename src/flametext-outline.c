...
{
	char *path = obs_module_file("effects/outline.effect");
	gs_effect_t *e = NULL;
	if (path) {
		e = gs_effect_create_from_file(path, NULL);
		if (!e)
			obs_log(LOG_ERROR, "failed to load outline.effect (%s)",
				path);
	}
	bfree(path);
	return e;
...
/* Push the shared params (texture, canvas, colour, width). */
		       ...
		       uint32_t ch, const float rgba[4], float width)
{
	...
	if ((p = gs_effect_get_param_by_name(e, "outline_color"))) {
		struct vec4 c;
		vec4_set(&c, rgba[0], rgba[1], rgba[2], rgba[3]);
		gs_effect_set_vec4(p, &c);
	}
	if ((p = gs_effect_get_param_by_name(e, "width")))
		gs_effect_set_float(p, width < 0.5f ? 0.5f : width);
}

void fx_outline_render_full(gs_effect_t *e, gs_texture_t *tex, uint32_t cw,
			    uint32_t ch, const float rgba[4], float width)
{
	if (!e || !tex || rgba[3] <= 0.0f || width <= 0.0f)
		return;
	set_params(e, tex, cw, ch, rgba, width);
			   ...
			   uint32_t x, uint32_t y, uint32_t cx, uint32_t cy)
{
	...
	/* Quad grown by the stroke width (plus a margin for the sampling disc)
	 * so the dilated outline is not clipped at the glyph rect. Local coords
	 * start at the glyph's top-left, matching fx_textfill_render_sub. */
	float m = width + 2.0f;
	float lx0 = -m, ly0 = -m;
