...
REM    package\font-effect-tools\  -> %OBS_DIR%\data\obs-plugins\font-effect-tools\
...
set OBS_DATA_DIR=%OBS_DIR%\data\obs-plugins\font-effect-tools
...
robocopy "%PKG_DIR%\font-effect-tools" "%OBS_DATA_DIR%" /MIR /R:1 /W:1 /NJH /NJS /NDL /NP
if errorlevel 8 goto :deploy_error

REM --- binaries: copy only our files (64bit\ is shared with other plugins) ---
echo   bin  -^> "%OBS_BIN_DIR%"
robocopy "%PKG_DIR%\bin" "%OBS_BIN_DIR%" font-effect-tools.dll font-effect-tools.pdb /R:1 /W:1 /NJH /NJS /NDL /NP
if errorlevel 8 goto :deploy_error

REM Verify the copies actually landed (robocopy's exit code can't be trusted).
if not exist "%OBS_BIN_DIR%\font-effect-tools.dll"  goto :deploy_error
